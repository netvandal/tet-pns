...
public class Validator {
	...
	/**
	 * Controlla l'esistenza e la validitˆ di tutti gli elementi
	 * di un place
	...
	public void placeController(String elementId, String nToken,
		...
		int token=0;
		try{
			p=new Place(); //la quantitˆ di token di default  0
			if(elementId==null)
				throw new InvalidFileException();
	...
	public void arcController(String elementId, String source, String target) throws InvalidFileException{
		...
		//Se source  un posto allora target deve essere una transizione
		if(s instanceof Place && !(t instanceof Transition))
			throw new InvalidFileException();
		
		//Se source  una transizione allora target deve essere un posto
